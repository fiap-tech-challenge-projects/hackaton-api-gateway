...
export class ProxyController {
  ...
  constructor(private readonly configService: ConfigService) {
    const uploadServiceUrl = this.configService.get<string>('app.uploadServiceUrl')
    const reportServiceUrl = this.configService.get<string>('app.reportServiceUrl')
  ...
  @All('/api/v1/docs')
  proxyDocs(@Req() req: Request, @Res() res: Response, @Next() next: NextFunction): void {
    this.uploadProxy(req, res, next)
  }
}
